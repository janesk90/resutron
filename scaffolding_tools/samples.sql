...
select s.skills_name, p.persons_firstname, p.persons_lastname from skills s join skills_to_persons stp on s.skills_id = stp.skills_id join persons p on stp.persons_id = p.persons_id order by s.skills_name asc;
